...
class WPertLRLinear(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x, _x):
        self._w = (torch.randn(self._w.shape[0], 1, device=self.w.device, dtype=self.w.dtype) * torch.randn(self._w.shape[1], device=self.w.device, dtype=self.w.dtype)).normal_()
        self._b = torch.randn_like(self._b, device=self.b.device, dtype=self.b.dtype).normal_()
        y = torch.matmul(x, self.w) + self.b
        _y = torch.matmul(x, self._w) + torch.matmul(_x, self.w) + self._b
        return y, _y
...
class WPertConv2d(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x, _x):
        self._kernel = torch.rand_like(self._kernel, device=self.kernel.device, dtype=self.kernel.dtype).normal_()
        self._bias = torch.rand_like(self._bias, device=self.bias.device, dtype=self.bias.dtype).normal_()
...
class WPertLRConv2d(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x, _x):
        self._kernel = (torch.rand(self._kernel.shape[0], 1, 1, 1, device=self.kernel.device, dtype=self.kernel.dtype) * 
                        torch.rand(self._kernel.shape[1], self._kernel.shape[2], self._kernel.shape[3], device=self.kernel.device, dtype=self.kernel.dtype)).normal_()
        self._bias = torch.rand_like(self._bias, device=self.bias.device, dtype=self.bias.dtype).normal_()
        y = F.conv2d(x, self.kernel, self.bias, stride=self.stride)
        _y = F.conv2d(_x, self.kernel, stride=self.stride) + F.conv2d(x, self._kernel, stride=self.stride) + self._bias.reshape(-1, 1, 1)
        return y, _y
...
class WPertAvgPool2d(nn.Module):
    @torch.no_grad()
